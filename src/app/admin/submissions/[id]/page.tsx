...
import { approveSubmissionAction, confirmRejectionAction, deleteTrashedAction, recommendRejectionAction, restoreNonpublishedAction, restorePublishedAction, setMovementWallVisibilityAction, trashSubmissionAction } from "@/app/admin/actions";
...
type DetailRecord = {
  id: string; status: string; display_name: string | null; submitted_at: string | null; guardian_number: number | null; show_on_movement_wall: boolean;
...
export default async function SubmissionDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ success?: string; cleanup?: string; testAction?: string }> }) {
  ...
  const successMessage =
    success === "published"
      ? `Published successfully. Guardian #${record.guardian_number} is now on the Movement Wall.`
      : success === "rejection-recommended"
        ? "Rejection recommendation saved and sent to an Admin for the final decision."
        : success === "rejected"
          ? "Submission rejected successfully."
          : success === "movement-visible"
            ? "This published promise is now displayed on the Movement Wall."
            : success === "movement-hidden"
              ? "This published promise is hidden from the Movement Wall. Its Guardian number and campaign count are unchanged."
          : null;
  ...
  return <>
    ...
    <div className="admin-detail-grid">
      ...
      <section className="admin-panel"><h2>Submission facts</h2><dl className="admin-facts"><div><dt>Status</dt><dd>{record.status.replaceAll("_", " ")}</dd></div><div><dt>Submitted</dt><dd>{record.submitted_at ? new Date(record.submitted_at).toLocaleString("en-IN") : "—"}</dd></div><div><dt>Guardian number</dt><dd>{record.guardian_number ? `#${record.guardian_number}` : "Not assigned"}</dd></div>{record.status === "published" && <div><dt>Movement Wall</dt><dd>{record.show_on_movement_wall ? "Displayed" : "Hidden"}</dd></div>}<div><dt>Publication consent</dt><dd>{consent?.publication_consent ? "Confirmed" : "Missing"}</dd></div><div><dt>Terms accepted</dt><dd>{consent?.terms_accepted ? "Confirmed" : "Missing"}</dd></div><div><dt>Image</dt><dd>{media?.original_width ?? "—"} × {media?.original_height ?? "—"} · {media?.original_mime_type ?? "unknown"} · {media?.original_bytes ? `${Math.round(media.original_bytes / 1024)} KB` : "—"}</dd></div>{result.session.role === "admin" && <div><dt>Participant email</dt><dd>{result.email ?? "Unavailable"}</dd></div>}<div><dt>Certificate</dt><dd>{certificate?.status?.replaceAll("_", " ") ?? "Not started"}</dd></div></dl>
    ...
    {record.status === "published" && !record.trashed_at && <section className="admin-panel admin-actions"><h2>Movement Wall visibility</h2><p>Choose whether this published photograph appears on the public Movement Wall. This does not change its Guardian number or the campaign count.</p><form action={setMovementWallVisibilityAction}><input type="hidden" name="submissionId" value={record.id} /><input type="hidden" name="visible" value={record.show_on_movement_wall ? "false" : "true"} /><button className={`button ${record.show_on_movement_wall ? "button--light" : "button--primary"}`} type="submit">{record.show_on_movement_wall ? "Hide from Movement Wall" : "Display on Movement Wall"}</button></form></section>}
