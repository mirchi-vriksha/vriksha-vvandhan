...
export default async function SubmissionDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ success?: string; cleanup?: string; testAction?: string }> }) {
  ...
  const decisionPanel = reviewable ? <section className="admin-panel admin-actions admin-decision-panel" aria-labelledby="moderation-decision-title">
    ...
    {result.session.role === "reviewer" && record.status === "pending_review" && <form action={recommendRejectionAction}><input type="hidden" name="submissionId" value={record.id} /><label>Reason shown to participant<select name="reasonCode" required defaultValue=""><option value="" disabled>Select a safe reason</option><option value="tree_or_rakhi_not_visible">Tree or Rakhi is not clearly visible</option><option value="image_quality">Image quality</option><option value="privacy_or_safety">Privacy or safety concern</option><option value="duplicate_submission">Duplicate submission</option><option value="campaign_mismatch">Does not match campaign guidelines</option><option value="other">Other campaign guideline</option></select></label><label>Optional participant guidance<textarea name="participantNote" maxLength={600} /></label><label>Internal moderation note<textarea name="internalNote" minLength={10} maxLength={1200} required /></label><button className="button button--light" type="submit">Recommend Rejection</button><small>An Admin makes the final decision. The internal note is never emailed.</small></form>}
