...
function fixtureData(rowCount = 1): CampaignExportData {
  ...
  return {
    ...
    certificates: submissions.filter((_, index) => index % 2 === 0).map((row) => ({ submission_id: row.id, status: "generated", template_version: "vriksha-2026-v1", object_path: `${row.id}/vriksha-guardian-1-v1.pdf`, file_bytes: 200000, attempt_count: 1, generated_at: "2026-08-07T09:01:00.000Z", last_error_code: null })),
