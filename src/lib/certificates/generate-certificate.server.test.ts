...
describe("certificate formatting", () => {
  it.each([[1, "1"], [27, "27"], [427, "427"], [983, "983"], [10234, "10234"]])(
  ...
  it("builds a private versioned path without PII", () => {
    ...
    expect(path).toBe("3f000000-0000-4000-8000-000000000001/vriksha-guardian-427-v3.pdf");
    expect(path).not.toContain("Jay");
    expect(buildCertificateFilename(427)).toBe("Vriksha-Guardian-427.pdf");
  });
});

describe("generateCertificate", () => {
  it("renders a valid deterministic 3:2 landscape PDF", async () => {
    ...
    expect(page.getWidth()).toBeCloseTo(864, 2);
    expect(page.getHeight()).toBeCloseTo(576, 2);
    expect(pdf.getAuthor()).toBe("Vriksha Bandhan by Mirchi");
    expect(pdf.getCreator()).toContain("vriksha-bandhan-2026-v3");
    expect(pdf.getSubject()).toContain("vriksha-bandhan-2026-v3");
  });
