...
describe("safe public campaign data", () => {
  ...
  it("validates summary and builds only public Storage URLs", async () => {
    ...
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(new Response(JSON.stringify([{ current_count: 3, target_count: 983, metric_label: "Vriksha promises", submissions_open: true, movement_wall_enabled: false }])))
    ...
    await expect(getPublicCampaignSummary()).resolves.toMatchObject({ current_count: 3, target_count: 983, movement_wall_enabled: false });
