...
Public submission receipts use Next.js `after()` for a best-effort attempt after the authoritative transaction succeeds. Approval and final-rejection Server Actions wait for their immediate certificate/email attempt before returning, so staff see a pending state and the success message can report whether the provider accepted the email. Database rows remain the durable truth: a failed or disabled immediate attempt never reverses moderation and stays visible in Deliveries for the bounded catch-up worker or an explicit Admin retry.
