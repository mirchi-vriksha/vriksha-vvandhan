...
Audit date: 14 August 2026. Scope: the effective schema after migration `20260814141500_email_workflow_reliability.sql`.
...
| `public.update_submission_review_fields`, `public.recommend_submission_rejection` | authenticated | REVIEWED / INTENTIONAL | Each derives the actor from `auth.uid()`, checks an active Reviewer role, validates bounded fields, locks the target state, and writes an audit event. Rejection reason, optional participant guidance and internal note are stored separately. |
...
| `public.claim_certificate_generation`, `public.complete_certificate_generation`, `public.fail_certificate_generation`, `public.claim_email_delivery`, `public.complete_email_delivery`, `public.fail_email_delivery` | service role only | REVIEWED / INTENTIONAL | Claim tokens, target-state checks, bounded attempts, safe error-code validation and exact object/provider metadata prevent client invocation and duplicate completion. Email claims also enforce the private suppression registry and move ambiguous expired work to manual review. Completion reconciles out-of-order provider events. |
...
| `public.record_resend_webhook_event` | service role only | FIXED | Stores verified provider event metadata and a bounded safe bounce class only, rejects invalid/future input, and is idempotent by event ID. It does not store the webhook body or recipient address. Permanent bounce, complaint and provider-suppressed outcomes propagate to the private suppression registry. |
| `public.begin_email_worker_run`, `public.complete_email_worker_run`, `public.purge_email_webhook_events` | service role only | REVIEWED / INTENTIONAL | Worker telemetry has bounded counters/outcomes, and cleanup enforces a 30–365 day retention bound plus a 5,000-row batch cap. No PII is returned. |
| `public.prepare_email_admin_retry`, `public.record_delivery_admin_action` | authenticated Admin only | REVIEWED / INTENTIONAL | Both derive the actor from `auth.uid()`, verify current Admin state and write an audit event. Explicit email retry increments the idempotency version before a new provider attempt. |
