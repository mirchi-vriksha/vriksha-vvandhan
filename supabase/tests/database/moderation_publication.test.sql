...
select throws_ok($$select public.recommend_submission_rejection('51000000-0000-4000-8000-000000000010','image_quality','Please retry with a clearer photograph.','A clear internal moderation note.')$$,'P0001','unauthorized_role','inactive staff cannot moderate');
...
select throws_ok($$select public.recommend_submission_rejection('51000000-0000-4000-8000-000000000010','not_allowed','','short')$$,'P0001','invalid_rejection_details','recommendation requires safe participant copy and an internal note');
select lives_ok($$select public.recommend_submission_rejection('51000000-0000-4000-8000-000000000010','image_quality','Please submit a clearer tree photograph.','The image is too blurred for recognition.')$$,'Reviewer recommends rejection');
...
select throws_ok($$select public.confirm_submission_rejection('51000000-0000-4000-8000-000000000010','image_quality','Please retry.','Reviewer must not confirm this.')$$,'P0001','unauthorized_role','Reviewer cannot confirm rejection');
...
select lives_ok($$select public.confirm_submission_rejection('51000000-0000-4000-8000-000000000011','tree_or_rakhi_not_visible','Please submit a photograph that clearly shows the tree and Rakhi.','This photograph does not clearly show a tree.')$$,'Admin rejects Pending Review directly');
select lives_ok($$select public.confirm_submission_rejection('51000000-0000-4000-8000-000000000011','tree_or_rakhi_not_visible','Please submit a photograph that clearly shows the tree and Rakhi.','This photograph does not clearly show a tree.')$$,'Final rejection is idempotent');
